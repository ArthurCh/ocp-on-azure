...
az vm create -g $RG_NAME --name ocp-master.$OCP_DOMAIN_SUFFIX --location westus --availability-set ocpAvailabilitySet --vnet-name ocpVnet --subnet ocpSubnet --image $VM_IMAGE --size $IMAGE_TYPE_MASTER --admin-username ocpuser --ssh-key-value ~/.ssh/id_rsa.pub

# Create the OCP Infra VM
echo "Creating the OCP Infra VM..."
az vm create -g $RG_NAME --name ocp-infra.$OCP_DOMAIN_SUFFIX --location westus --availability-set ocpAvailabilitySet --vnet-name ocpVnet --subnet ocpSubnet --image $VM_IMAGE --size $IMAGE_TYPE_INFRA --admin-username ocpuser --ssh-key-value ~/.ssh/id_rsa.pub
